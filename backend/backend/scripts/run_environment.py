    ...
    def run_algorithm(self):
        ...
        try:
            proc = subprocess.Popen(
                [f"python3 {self.temp_alg_file} {self.temp_data_file}"], shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

            # Collect output and write to file
            self.collect_ouput(proc)

            print(f"Output file can be found in {os.path.join(os.getcwd(), self.download_dir)}")
