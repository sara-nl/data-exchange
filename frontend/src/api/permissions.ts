...
export type PermissionType = "one time permission" | 
"stream permission" | 
...
export type Permission = {
    ...
    algorithm_report?: {info: object, totals: object},
    algorithm_provider: string,
    dataset: string,
    dataset_provider: string,
    state: "analyzing" | "pending" | "cancelled" | "rejected" | "active" | "aborted",
    registered_on: Date,
    created_on: Date,
    updated_on: Date,
...
export async function approvePermission(id: number, dataset: string): Promise<Permission> {
    const response = await Controller.client.put<Permission>(`/permissions/${id}/approve/`, {dataset})
    return response.data
}

export async function rejectPermission(id: number): Promise<Permission> {
    const response = await Controller.client.put<Permission>(`/permissions/${id}/reject/`)
    return response.data
}

export async function headTaskId(id: number): Promise<{id: number} | null> {
    const response = await Controller.client.get<{id: number} | null>(`/permissions/${id}/head_task_id/`)
    return response.data
}
...
export const permissionApprovalActionDict = {
    ...
    "One specific user permission": "Give Permission to run any algorithm on dataset",
}

export const permissionTypeDict = {
    "algorithm": {
        "one time permission": "Selected algorithm will be ran on the selected dataset of the data owner exactly once.",
        "stream permission": "Every change in the selected data (or data folder) will automatically start a new run using this algorithm.",
        "One specific user permission": "You can always run all the algorithms you've shared to DataExchange on the selected dataset.",
    },
    "data": {
        "one time permission": "Selected algorithm will be ran on your selected dataset once.",
        "stream permission": "Every change in the selected data (or data folder) will automatically start a new run using the given algorithm.",
        "One specific user permission": "The requesting user can run  all his algorithms on the dataset you're going to select at any time.",
    }
}

export function permissionInfo(permissionType: PermissionType, role: UserRole): string {
