...
import nl.surf.dex.storage.gdrive.GDriveFileset.{downloadGFile, listChildFilesOf}
...
object GDriveFileset {
  ...
  private[gdrive] def listShares: Kleisli[IO, Drive, List[GFile]] =
    Kleisli { drive =>
      IO(
        drive
          .files()
  ...
  private[gdrive] def downloadGFile(file: GFile, localParent: BFile)(implicit
      cs: ContextShift[IO]
  ): Kleisli[IO, Drive, BFile] =
    Kleisli { drive =>
        ...
        case _ if file.getSize > 0 =>
          IO(localParent.createDirectories()) *>
            Resource
              ...
              .use(os => IO(drive.files().get(file.getId).executeMediaAndDownloadTo(os))) *> IO(
            destFile
          )
        case _ =>
  ...
  private def assumeDirectory(parent: GFile): IO[Unit] =
    parent match {
...
class GDriveFileset(driveIO: IO[Drive]) extends FilesetOps {
  ...
  def getFilesetHash(userPath: Share.NePath): IO[Fileset.Hash] =
    driveIO.flatMap {
      (for {
        ...
        children <-
          if (GDriveFileset.isDirectory(share)) {
  ...
  override def copySharedFileset(path: Share.NePath, localParent: BFile)(implicit
      cs: ContextShift[IO]
  ): IO[BFile] =
    driveIO.flatMap {
      (path.segments match {
