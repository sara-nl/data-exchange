...
class SecureContainerFlow(
    consumer: Stream[IO, AmqpEnvelope[String]],
...
//  val jsonEncodePipe: Pipe[IO, AmqpMessage[Messages.StartContainer], AmqpMessage[String]] = _.map(jsonEncode[Messages.StartContainer])
  val jsonDecodePipe
      : Pipe[IO, AmqpEnvelope[String], Either[io.circe.Error, Messages.StartContainer]] =
    _.map(env => decode[Messages.StartContainer](env.payload))

  val executeCodePipe: Pipe[IO, Messages.StartContainer, (String, Either[String, String])] =
    _.evalMap { msg =>
      Webdav.codeAndDataResources(msg.codePath, msg.dataPath).use {
        case (tempDir, codeIS, dataIS) =>
          ...
          val joinLines: (String, String) => String = {
            case (l1: String, l2: String) => s"$l1\n$l2"
          }
          ...
          for {
            ...
            containerId <- SecureContainer
              .createContainer(codeHome, dataHome, msg.codePath, msg.dataPath)
            ...
            (msg.taskId, stateAndCode match {
              case Some((_, 0)) =>
                outputOption.toRight(s"Could not get status and logs of container ${containerId}")
