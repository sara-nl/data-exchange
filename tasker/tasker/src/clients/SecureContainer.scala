...
object SecureContainer {
  ...
  def outputStream(containerId: String)(implicit F: ConcurrentEffect[IO], cs: ContextShift[IO]): fs2.Stream[IO, String] = {
    import fs2._
    import fs2.concurrent._

    def allLogsCommand(containerId: String) = client.logContainerCmd(containerId)
  ...
  def statusesAndLogs(contId: String)(implicit F: ConcurrentEffect[IO], sc: ContextShift[IO]) = {
    val statusStream = SecureContainer
      .statusStream(contId).map(v => s"Container status: $v")
